import { createOjRepository } from "./repositories/oj-repository.mjs";
...
export function createDatabase(databaseUrl) {
  ...
  return {
    async migrate() {
      ...
      try {
        await client.query(
          "SELECT pg_advisory_lock(hashtext('algoquest_schema_migrations'))",
        );
        await client.query(`
        ...
        try {
          await client.query(
            "SELECT pg_advisory_unlock(hashtext('algoquest_schema_migrations'))",
          );
        } finally {
          client.release();
        }
